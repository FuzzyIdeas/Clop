...
PDF optimisation now exposes the image resolution that Ghostscript downsamples to.

- The minus button on a PDF result opens a slider with stops at `300`, `250`, `200`, `150`, `100`, `72` and `48` DPI. Drag to re-run the optimisation at the chosen DPI.
- Two new settings under **PDF** let you set the default DPI for normal and aggressive optimisation. `300` means no downsampling.
- Pipelines accept a `dpi` parameter on the optimise step, e.g. `optimise(encoder: aggressive, dpi: 150)`.
